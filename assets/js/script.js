...
const tittleLength = document.getElementById("tittleLength");
       ...
       <div class="p-3">
         <div class="card" >
           ...
           <span class="mx-4 ${
            ...
            <p class="card-text text-start">${issue.tittle}</p>
...
const formValidate = (e) => {
  e.preventDefault();
  if (tittle.value === "") {
    alert("Please enter task tittle");
    tittle.focus();
    return;
  }
  if (tittle.value.length < 60 || tittle.value.trim().length < 60) {
    alert("Tittle should be atleast 60 charecters");
    tittle.focus();
    return;
  }
...
function textCounter() {
  let count = tittle.value.length;
  tittleLength.innerHTML = `${maxLength - count} charecters left`;
  tittleLength.classList.remove("text-danger");
  if (count == "") {
  ...
  if (count > warnLength) {
    tittleLength.classList.add("text-danger");
    tittleLength.innerHTML = `${maxLength - count} charecters left`;
  }
}
...
function textCounter() {
  ...
  if (count == "") {
    descriptionLength.innerHTML = `${maxLength} charecters left`;
  }
